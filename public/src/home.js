...
function getMostPopularAuthors(books, authors) {
  ...
  for (author of authors){//go through the list of authors
    let count = 0;// initiate count for the author's books' count of borrowed
    let booksByAuthor = bookPopularity.filter((book) => book.authorId === author.id); // find books of the author
    for(bookByAuthor of booksByAuthor){ //go through each book of the author and add up the amount of times borrowed
      count += bookByAuthor.count;//accumulate the count
    }
    authorBookCount = {'name':`${author.name.first} ${author.name.last}`, 'count':count};//create the object of the author with their total count from their books
    authorPopularity.push(authorBookCount);//push the object to the array of authors 
  }
